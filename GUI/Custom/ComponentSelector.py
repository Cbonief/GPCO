...
from GUI.Custom.selectComponentWindow import Ui_ComponentSelectWindow
from GUI.Custom.selectSingleComponentWindow import Ui_SingleComponentSelectWindow
...
class SingleComponentSelector(Ui_SingleComponentSelectWindow):
    ...
    def add_component(self):
        if self.selected_item is not None and len(
            ...
            self.available_components[self.component_being_selected].sort()
    ...
    def get_selected_components(self):
        ...
        for component in self.selected_components:
            try:
                output[component] = self.selected_components[component][0]
            except IndexError:
                output[component] = None
        return output

    def set_selected_components(self, selected):
        self.reset()
        self.configuration_complete = True
        for component in selected:
            if selected[component] is not None:
                self.available_components[component].remove(selected[component])
                self.selected_components[component].append(selected[component])
            self.available_components[component].sort()
            if len(self.selected_components[component]) == 0:
                self.configuration_complete = False
    ...
    def reset(self):
        for component in self.selected_components:
            if len(self.selected_components[component]) > 0:
                self.available_components[component].append(self.selected_components[component][0])
...
class MultiComponentSelector(Ui_ComponentSelectWindow):
    ...
    def add_component(self):
        if self.selected_item is not None and self.selected_item.text() in self.available_components[
            ...
            self.available_components[self.component_being_selected].remove(self.selected_item.text())
            self.selected_components[self.component_being_selected].append(self.selected_item.text())
            self.selected_components[self.component_being_selected].sort()
            self.model_available = QStandardItemModel(self.list_available)
            for name in self.available_components[self.component_being_selected]:
                item = QStandardItem(name)
                item.setEditable(False)
    ...
    def set_selected_components(self, selected):
        ...
        self.configuration_complete = True
        for component in selected:
            for key in selected[component]:
                self.available_components[component].remove(key)
                self.selected_components[component].append(key)
            self.available_components[component].sort()
            self.selected_components[component].sort()
            if len(self.selected_components[component]) == 0:
                self.configuration_complete = False
    ...
    def reset(self):
        for component in self.selected_components:
            for key in self.selected_components[component]:
                self.available_components[component].append(key)
            self.selected_components[component] = []
            self.available_components[component].sort()
        self.configuration_complete = False
