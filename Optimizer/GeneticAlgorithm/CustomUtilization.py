...
def converter_from_genome(genome, design_features, safety_parameters):
    ...
    converter = BoostHalfBridgeInverter(design_features, safety_parameters, transformer, entrance_inductor, auxiliary_inductor, switches, diodes, capacitors)
    return converter
...
def generate_genome(available_genes, args):
    ...
    while not bounds_feasible:
        ...
        while not feasible:
            ...
            cables = [np.random.choice(available_genes['Primary Cable']), np.random.choice(available_genes['Secondary Cable'])]
            found = False
            n = [0, 0]
            while not found:
        ...
        while not feasible:
            ...
            ncond = np.random.randint(available_genes['Entrance Inductor Ncond'][0], available_genes['Entrance Inductor Ncond'][1])
        ...
        while not feasible:
            ...
            n = np.random.randint(available_genes['Auxiliary Inductor N'][0], available_genes['Auxiliary Inductor N'][1])
            ncond = np.random.randint(available_genes['Auxiliary Inductor Ncond'][0], available_genes['Auxiliary Inductor Ncond'][1])
        ...
        test_converter = BoostHalfBridgeInverter(design_features, safety_parameters, transformer, entrance_inductor, auxiliary_inductor, switches, diodes, capacitors)
...
def optimize_components(selected_components_keys, components_data_base, design_features, safety_parameters, ga_config=None, num_opt_config=None):
